...
func handler(upgrader *websocket.Upgrader,
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			if err != nil {
				for k, v := range fhttpReq.Header {
					logger.Debug("---header:--->", k, "---value:--->", v)
				}
				logger.Debug("---data length---", len(fhttpReq.Data))
				logger.Error(err, ", will close connection")
				return
			}
			// solve data+header !=4 error
			fhttpReq.Header["X-Real-IP"] = conn.RemoteAddr().String()
			for k, v := range fhttpReq.Header {
				logger.Debug("---header:-->", k, "---value:-->", v)
			}
			if yes {
